...
- 字体为 Inter（拉丁）+ Noto Sans SC（中文），均经 Astro Fonts API 自托管；Noto SC 按 unicode-range 分片按需加载，不做 preload。
- 按钮、卡片、图片和表单使用直角、`2px` 黑色（深色模式白色）边框，以及固定偏移的硬阴影。
- 页面背景保留低对比方格底纹；标题字距收紧到 `-0.02em`（中文可读的安全值，勿再加大）；键盘焦点使用清晰的黄色描边。
- 彩色拼贴区块（如首页 Hero 卡片、增长系统图）内的浅色面板使用固定米色 `#fffcf2`，不跟随深色模式变深，保证黑色文字与图形始终可读。
